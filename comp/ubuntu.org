...
gsettings set com.canonical.Unity.Panel systray-whitelist "['all']"
   # system
   sudo apt-get install unrar stardict sdcv curl gnome-shell gnome-tweak-tool synergy quicksynergy
   sudo apt-get install ibus-pinyin ibus-pinyin-db-android
   sudo apt-get install hardinfo
   # vc
   sudo apt-get install git bzr gitk qgit gitg cvs

   # human
   # emacs 
#    sudo ln -s /home/s/nethd/local_soft/emacs/src/emacs /usr/bin
#    sudo apt-get install ispell iamerican dictionaries-common mew
#    echo "lang en" >> ~/.aspell.conf
#    sudo apt-get install autoconf automake  libgtk-3-dev  libxpm-dev libjpeg-dev libgif-dev libtiff4-dev texinfo libtool libncurses-dev
#    ./autogen.sh
...
#   sudo add-apt-repository ppa:eugenesan/java # oracle-java7-installer
   sudo add-apt-repository ppa:webupd8team/gnome3 # gnome-shell
...
#   sudo apt-get install sun-java6-jre sun-java6-jdk sun-java6-plugin
   ...
   sudo apt-get install gnome-shell-extensions gnome-shell-extensions-weather gnome-shell-system-monitor gnome-shell-extensions-gpaste gnome-shell-extensions-mediaplayer gnome-shell-classic-systray gnome-shell-message-notifier gnome-shell-extension-notesearch
   ...
   sudo apt-get install keynav # tooles
   # DB
   sudo apt-get install mysql-client mysql-workbench mongodb sqlite3 redis-server mysql-workbench  memcached nodejs libmysqlclient-dev libreadline-dev libsqlite3-dev libxml2-dev libxslt-dev libcurl4-gnutls-dev g++ libssl-dev curl  git bzr gitk qgit autoconf automake libgtk2.0-dev libxpm-dev libjpeg-dev libgif-dev libtiff4-dev texinfo

   # ror lib dependence
   sudo apt-get install libmysqlclient-dev libreadline-dev libsqlite3-dev libxml2-dev libxslt-dev libcurl4-gnutls-dev g++ libssl-dev libpq-dev libmagickwand-dev
   sudo apt-get install imagemagick # 图片工具
   sudo apt-get install exuberant-ctags # 增强的ctags ctags-exuberant -a -e -f TAGS --languages=Ruby --tag-relative -R app config lib vendor ~/.rvm/rubies/default/lib/ruby/gems/1.9/gems ~/.rvm/src/ruby-1.9.3-p194
   # ctags -e -R --extra=+fq --exclude=db --exclude=doc --exclude=log --exclude=tmp --exclude=.git --exclude=public . $(rvm gemdir)/gems
   # ctags -e -a --Ruby-kinds=fF -o TAGS app test lib config script bin
   # ctags -e -R --extra=+fq --exclude=db --exclude=test --exclude=.git --exclude=public -f TAGS
   # ripper-tags -R --exclude=vendor -e
   # ripper-tags -R -e ./ /home/sjm/.rvm/gems/ruby-1.9.3-p392/gems
   gem install rtags # ruby tags, need >=0.89, using:  rtags `find ./app ./config ./db/ ./lib ./vendor ~/.rvm/gems/ruby-1.9.3-p392/gems/ ~/.rvm/src/ruby-1.9.3-p392/ -name *.rb | grep -v _darcs`
   sudo apt-get install global # gtags, need gtags.el for emacs, go to ror dir, and using: ln -s ~/.rvm/src/ruby-1.9.3-p194/ ./tmp/ruby  & ln -s ~/.rvm/gems/ruby-1.9.3-p194/gems/ ./tmp/gems & sleep 1 ; gtags -v --gtagslabel=rtags

;;; start ;;; gtags, for GNU Global
;;    for ruby: using rtags gem and set: export GTAGSLABEL=rtags
...
(add-hook 'ruby-mode-hook (lambda ()
   ...
   echo "                               " >> ~/.bash_profile
   echo "python /home/sjm/program/wall/goagent/local/proxy.py &" >> ~/.bash_profile
   echo "# ruby set" >> ~/.bash_profile
   echo 'export RUBYOPT="-KU -E utf-8:utf-8"' >> ~/.bash_profile
   echo "# emacs host                   " >> ~/.bash_profile
   echo "                               " >> ~/.bashrc
   echo "# emacs client                 " >> ~/.bashrc
   echo "alias et='emacsclient -t'      " >> ~/.bashrc
   echo "alias ec='emacsclient -c &'    " >> ~/.bashrc
   echo "alias es='export LANG=en_US.UTF-8 && emacs --daemon'" >> ~/.bashrc

# rvm set
[[ -s "$HOME/.rvm/scripts/rvm" ]] && . "$HOME/.rvm/scripts/rvm" # This loads RVM into a shell session


   # GRANT ALL PRIVILEGES ON *.* TO tianji@localhost IDENTIFIED BY 'tianji' WITH GRANT OPTION; # mysql add user
   ...
   sudo echo "" >> /etc/fstab
   sudo echo "/dev/sda1 /media/c ntfs user, nls=utf8, umask=0222, gid=1000, auto 0" >> /etc/fstab
   sudo echo "/dev/sda5 /media/tools ntfs user, nls=utf8, umask=0222, gid=1000, auto 0" >> /etc/fstab
   sudo echo "/dev/sda6 /media/windata ntfs user, nls=utf8, umask=0222, gid=1000, auto 0" >> /etc/fstab
   sudo echo "/dev/sda7 /media/linuxwin vfat user, auto, umask=002, gid=1000, utf8 0 0" >> /etc/fstab
   sudo echo "/dev/sda11 /media/data ntfs user, nls=utf8, umask=0222, gid=1000, auto 0" >> /etc/fstab
...
sudo apt-get update &&
sudo apt-get install zukitwo-gtk-theme zukitwo-dark-gtk-theme faenza-icon-theme indicator-weather jupiter calendar-indicator indicator-multiload # 系统监控


** keyboard share: synergy
...
** ror
   ...
   # stard test
   sudo apt-get install factory_girl # fixture
   # end   test
...
*** ape http://www.webupd8.org/2011/12/lossless-audio-split-tools-updates.html
sudo add-apt-repository ppa:flacon/ppa
sudo apt-get update
sudo apt-get install flacon

*** mplayer-vaapi  http://www.linuxidc.com/Linux/2012-11/74916.htm
...
Ubuntu 12.10, 12.04 or 11.10用户可以通过下面的命令安装支持ＶＡ-API的Mplayer.
...
**** find . -regex ".*\.\(txt\|sh\)"
**** find . -regex ".*\.\(html.erb\|rhtml\|rb\|yml\)" | xargs grep -ir administrator
**** ps grep awk
...
***** git submodule add repository local-dir
      sjm@sjm:~/.emacs.d$ git submodule add git://github.com/pkg-ime/ibus-el.git ./personal/package/ibus-el
      git submodule add git://github.com/austin-----/weibo.emacs.git ./personal/package/weibo
***** git add .gitmodules personal/package/ibus-el
***** git commit -m "add submodule"
***** git submodule init
***** cd personal/package/ibus-el && git pull origin master
***** git rm --cached personal/package/ibus-el
***** rm -rf personal/package/ibus-el
***** edit .gitmodule
***** edit .git/config
***** git add .gitmodule
...
1.到官网下载安装包： jdk-7-linux-i586.tar.gz

2.创建安装目录：sudo mkdir /usr/lib/jvm

3. 解压缩：tar zxvf ./jdk-7-linux-i586.tar.gz -C /usr/lib/jvm
...
** linux user
*** useradd s -g Administrator -G sudo,adm,sambashare -d /home/s
*** passwd s
*** ln
ln -s /home/prj/program/ ~/program
ln -s /home/prj/rvm/ ~/.rvm
